...
pub export fn make_server_socket(arg_host: [*c]u8, arg_port: [*c]u8) c_int {
    var host = arg_host;
    _ = &host;
    var port = arg_port;
    _ = &port;
    if ((host != null) and !(strncmp(host, "unix:", @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 5))))) != 0)) {
        return make_unix_socket(&host[@as(c_uint, @intCast(@as(c_int, 5)))]);
    } else {
...
pub extern fn vfprintf(noalias __s: [*c]FILE, noalias __format: [*c]const u8, __arg: [*c]struct___va_list_tag_12) c_int;
...
pub extern fn vfscanf(noalias __s: [*c]FILE, noalias __format: [*c]const u8, __arg: [*c]struct___va_list_tag_12) c_int;
...
pub const struct___locale_data_17 = opaque {};
pub const struct___locale_struct = extern struct {
    __locales: [13]?*struct___locale_data_17 = @import("std").mem.zeroes([13]?*struct___locale_data_17),
...
pub fn set_nonblocking(arg_fd: c_int) callconv(.C) c_int {
    var fd = arg_fd;
    _ = &fd;
    ...
    if (flags < @as(c_int, 0)) {
        warn("%s:%d in %s: getting flags%s", "src/submodules/beanstalkd/net.c", @as(c_int, 25), "set_nonblocking", "");
        return -@as(c_int, 1);
    }
    r = fcntl(fd, @as(c_int, 4), flags | @as(c_int, 2048));
    if (r == -@as(c_int, 1)) {
        warn("%s:%d in %s: setting O_NONBLOCK%s", "src/submodules/beanstalkd/net.c", @as(c_int, 30), "set_nonblocking", "");
        return -@as(c_int, 1);
    }
    return 0;
}
pub fn make_inet_socket(arg_host: [*c]u8, arg_port: [*c]u8) callconv(.C) c_int {
    var host = arg_host;
    _ = &host;
    var port = arg_port;
    _ = &port;
    ...
    var r: c_int = undefined;
    _ = &r;
    var linger_1: struct_linger = struct_linger{
        .l_onoff = @as(c_int, 0),
        .l_linger = @as(c_int, 0),
    };
    _ = &linger_1;
    var airoot: [*c]struct_addrinfo = undefined;
    _ = &airoot;
    var ai: [*c]struct_addrinfo = undefined;
    _ = &ai;
    ...
    if (r != @as(c_int, 0)) {
        warnx("%s:%d in %s: getaddrinfo(): %s%s", "src/submodules/beanstalkd/net.c", @as(c_int, 49), "make_inet_socket", gai_strerror(r), "");
        return -@as(c_int, 1);
    }
    {
        ai = airoot;
        while (ai != null) : (ai = ai.*.ai_next) {
            ...
            if (fd == -@as(c_int, 1)) {
                warn("%s:%d in %s: socket()%s", "src/submodules/beanstalkd/net.c", @as(c_int, 56), "make_inet_socket", "");
                continue;
            }
            ...
            flags = 1;
            r = setsockopt(fd, @as(c_int, 1), @as(c_int, 2), @as(?*const anyopaque, @ptrCast(&flags)), @as(socklen_t, @bitCast(@as(c_uint, @truncate(@sizeOf(c_int))))));
            if (r == -@as(c_int, 1)) {
                warn("%s:%d in %s: setting SO_REUSEADDR on fd %d%s", "src/submodules/beanstalkd/net.c", @as(c_int, 69), "make_inet_socket", fd, "");
                _ = close(fd);
                continue;
            }
            r = setsockopt(fd, @as(c_int, 1), @as(c_int, 9), @as(?*const anyopaque, @ptrCast(&flags)), @as(socklen_t, @bitCast(@as(c_uint, @truncate(@sizeOf(c_int))))));
            if (r == -@as(c_int, 1)) {
                warn("%s:%d in %s: setting SO_KEEPALIVE on fd %d%s", "src/submodules/beanstalkd/net.c", @as(c_int, 75), "make_inet_socket", fd, "");
                _ = close(fd);
                continue;
            }
            r = setsockopt(fd, @as(c_int, 1), @as(c_int, 13), @as(?*const anyopaque, @ptrCast(&linger_1)), @as(socklen_t, @bitCast(@as(c_uint, @truncate(@sizeOf(struct_linger))))));
            if (r == -@as(c_int, 1)) {
                warn("%s:%d in %s: setting SO_LINGER on fd %d%s", "src/submodules/beanstalkd/net.c", @as(c_int, 81), "make_inet_socket", fd, "");
                _ = close(fd);
                continue;
            }
            r = setsockopt(fd, IPPROTO_TCP, @as(c_int, 1), @as(?*const anyopaque, @ptrCast(&flags)), @as(socklen_t, @bitCast(@as(c_uint, @truncate(@sizeOf(c_int))))));
            if (r == -@as(c_int, 1)) {
                warn("%s:%d in %s: setting TCP_NODELAY on fd %d%s", "src/submodules/beanstalkd/net.c", @as(c_int, 87), "make_inet_socket", fd, "");
                _ = close(fd);
                continue;
            }
            if ((host == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) and (ai.*.ai_family == @as(c_int, 10))) {
                flags = 0;
                r = setsockopt(fd, IPPROTO_IPV6, @as(c_int, 26), @as(?*const anyopaque, @ptrCast(&flags)), @as(socklen_t, @bitCast(@as(c_uint, @truncate(@sizeOf(c_int))))));
                if (r == -@as(c_int, 1)) {
                    warn("%s:%d in %s: setting IPV6_V6ONLY on fd %d%s", "src/submodules/beanstalkd/net.c", @as(c_int, 96), "make_inet_socket", fd, "");
                    _ = close(fd);
                    continue;
                }
            }
            ...
            if (r == -@as(c_int, 1)) {
                warn("%s:%d in %s: bind()%s", "src/submodules/beanstalkd/net.c", @as(c_int, 104), "make_inet_socket", "");
                _ = close(fd);
                continue;
            }
            if (verbose != 0) {
                var hbuf: [1025]u8 = undefined;
                _ = &hbuf;
                ...
                _ = &addrlen;
                addrlen = @as(socklen_t, @bitCast(@as(c_uint, @truncate(@sizeOf(struct_sockaddr_in)))));
                r = getsockname(fd, @as([*c]struct_sockaddr, @ptrCast(@alignCast(&addr))), &addrlen);
                if (!(r != 0)) {
                    r = getnameinfo(@as([*c]struct_sockaddr, @ptrCast(@alignCast(&addr))), addrlen, @as([*c]u8, @ptrCast(@alignCast(&hbuf))), @as(socklen_t, @bitCast(@as(c_uint, @truncate(@sizeOf([1025]u8))))), @as([*c]u8, @ptrCast(@alignCast(&pbuf))), @as(socklen_t, @bitCast(@as(c_uint, @truncate(@sizeOf([32]u8))))), @as(c_int, 1) | @as(c_int, 2));
                    if (!(r != 0)) {
                        h = @as([*c]u8, @ptrCast(@alignCast(&hbuf)));
                        p = @as([*c]u8, @ptrCast(@alignCast(&pbuf)));
                    }
                }
                if (ai.*.ai_family == @as(c_int, 10)) {
                    _ = printf("bind %d [%s]:%s\n", fd, h, p);
                } else {
                    _ = printf("bind %d %s:%s\n", fd, h, p);
                }
            }
            r = listen(fd, @as(c_int, 1024));
            if (r == -@as(c_int, 1)) {
                warn("%s:%d in %s: listen()%s", "src/submodules/beanstalkd/net.c", @as(c_int, 134), "make_inet_socket", "");
                _ = close(fd);
                continue;
            }
            break;
    ...
    freeaddrinfo(airoot);
    if (ai == @as([*c]struct_addrinfo, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        fd = -@as(c_int, 1);
    }
    return fd;
}
pub fn make_unix_socket(arg_path: [*c]u8) callconv(.C) c_int {
    var path = arg_path;
    _ = &path;
    var fd: c_int = -@as(c_int, 1);
    _ = &fd;
    ...
    _ = &addr;
    const maxlen: usize = @sizeOf([108]u8) -% @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1))));
    _ = &maxlen;
    _ = memset(@as(?*anyopaque, @ptrCast(&addr)), @as(c_int, 0), @sizeOf(struct_sockaddr_un));
    addr.sun_family = 1;
    if (strlen(path) > maxlen) {
        warnx("socket path %s is too long (%ld characters), where maximum allowed is %ld", path, strlen(path), maxlen);
        return -@as(c_int, 1);
    }
    _ = strncpy(@as([*c]u8, @ptrCast(@alignCast(&addr.sun_path))), path, maxlen);
    r = stat(path, &st);
    if (r == @as(c_int, 0)) {
        if ((st.st_mode & @as(__mode_t, @bitCast(@as(c_int, 61440)))) == @as(__mode_t, @bitCast(@as(c_int, 49152)))) {
            warnx("removing existing local socket to replace it");
            r = unlink(path);
            if (r == -@as(c_int, 1)) {
                warn("%s:%d in %s: unlink%s", "src/submodules/beanstalkd/net.c", @as(c_int, 173), "make_unix_socket", "");
                return -@as(c_int, 1);
            }
        } else {
            warnx("%s:%d in %s: another file already exists in the given path%s", "src/submodules/beanstalkd/net.c", @as(c_int, 177), "make_unix_socket", "");
            return -@as(c_int, 1);
        }
    }
    fd = socket(@as(c_int, 1), SOCK_STREAM, @as(c_int, 0));
    if (fd == -@as(c_int, 1)) {
        warn("%s:%d in %s: socket()%s", "src/submodules/beanstalkd/net.c", @as(c_int, 184), "make_unix_socket", "");
        return -@as(c_int, 1);
    }
    r = set_nonblocking(fd);
    if (r == -@as(c_int, 1)) {
    ...
    r = bind(fd, @as([*c]struct_sockaddr, @ptrCast(@alignCast(&addr))), @as(socklen_t, @bitCast(@as(c_uint, @truncate(@sizeOf(struct_sockaddr_un))))));
    if (r == -@as(c_int, 1)) {
        warn("%s:%d in %s: bind()%s", "src/submodules/beanstalkd/net.c", @as(c_int, 196), "make_unix_socket", "");
        _ = close(fd);
        return -@as(c_int, 1);
    }
    if (verbose != 0) {
    ...
    r = listen(fd, @as(c_int, 1024));
    if (r == -@as(c_int, 1)) {
        warn("%s:%d in %s: listen()%s", "src/submodules/beanstalkd/net.c", @as(c_int, 206), "make_unix_socket", "");
        _ = close(fd);
        return -@as(c_int, 1);
    }
    return fd;
