...
pub export fn allocate_job(arg_body_size: c_int) [*c]Job {
    var body_size = arg_body_size;
    _ = &body_size;
    var j: [*c]Job = undefined;
    _ = &j;
    j = @as([*c]Job, @ptrCast(@alignCast(malloc(@sizeOf(Job) +% @as(c_ulong, @bitCast(@as(c_long, body_size)))))));
    if (!(j != null)) {
        warnx("%s:%d in %s: OOM%s", "src/submodules/beanstalkd/job.c", @as(c_int, 100), "allocate_job", "");
        return @as([*c]Job, @ptrFromInt(@as(c_int, 0)));
    }
    _ = memset(@as(?*anyopaque, @ptrCast(j)), @as(c_int, 0), @sizeOf(Job));
    j.*.r.created_at = nanoseconds();
    j.*.r.body_size = body_size;
    j.*.body = @as([*c]u8, @ptrCast(@alignCast(j))) + @sizeOf(Job);
    job_list_reset(j);
    return j;
}
pub export fn make_job_with_id(arg_pri: uint, arg_delay: int64, arg_ttr: int64, arg_body_size: c_int, arg_tube: [*c]Tube, arg_id: uint64) [*c]Job {
    var pri = arg_pri;
    _ = &pri;
    var delay = arg_delay;
    _ = &delay;
    ...
    if (!(j != null)) {
        warnx("%s:%d in %s: OOM%s", "src/submodules/beanstalkd/job.c", @as(c_int, 120), "make_job_with_id", "");
        return @as([*c]Job, @ptrFromInt(@as(c_int, 0)));
    }
    if (id != 0) {
        j.*.r.id = id;
        if (id >= next_id) {
            next_id = id +% @as(uint64, @bitCast(@as(c_long, @as(c_int, 1))));
        }
    } else {
        j.*.r.id = blk: {
            const ref = &next_id;
    ...
    _ = blk: {
        ...
        break :blk tube_iref(j.*.tube);
    };
    return j;
}
pub export fn job_free(arg_j: [*c]Job) void {
    var j = arg_j;
    _ = &j;
    if (j != null) {
        _ = blk: {
        ...
        if (@as(c_int, @bitCast(@as(c_uint, j.*.r.state))) != Copy) {
            job_hash_free(j);
        }
    }
    free(@as(?*anyopaque, @ptrCast(j)));
}
pub export fn job_find(arg_job_id: uint64) [*c]Job {
    var job_id = arg_job_id;
    _ = &job_id;
    var index_1: c_int = _get_job_hash_index(job_id);
    _ = &index_1;
    var jh: [*c]Job = (blk: {
        const tmp = index_1;
        if (tmp >= 0) break :blk all_jobs + @as(usize, @intCast(tmp)) else break :blk all_jobs - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    }).*;
    _ = &jh;
    while ((jh != null) and (jh.*.r.id != job_id)) {
        jh = jh.*.ht_next;
    }
    return jh;
}
pub export fn job_setpos(arg_j: ?*anyopaque, arg_pos: usize) void {
    var j = arg_j;
    _ = &j;
    var pos = arg_pos;
    _ = &pos;
    @as([*c]Job, @ptrCast(@alignCast(j))).*.heap_index = pos;
}
pub export fn job_pri_less(arg_ja: ?*anyopaque, arg_jb: ?*anyopaque) c_int {
    var ja = arg_ja;
    _ = &ja;
    var jb = arg_jb;
    _ = &jb;
    var a: [*c]Job = @as([*c]Job, @ptrCast(@alignCast(ja)));
    _ = &a;
    var b: [*c]Job = @as([*c]Job, @ptrCast(@alignCast(jb)));
    _ = &b;
    if (a.*.r.pri < b.*.r.pri) return 1;
    if (a.*.r.pri > b.*.r.pri) return 0;
    return @intFromBool(a.*.r.id < b.*.r.id);
}
pub export fn job_delay_less(arg_ja: ?*anyopaque, arg_jb: ?*anyopaque) c_int {
    var ja = arg_ja;
    _ = &ja;
    var jb = arg_jb;
    _ = &jb;
...
pub export fn job_copy(arg_j: [*c]Job) [*c]Job {
    var j = arg_j;
    _ = &j;
    if (!(j != null)) return null;
    var n: [*c]Job = @as([*c]Job, @ptrCast(@alignCast(malloc(@sizeOf(Job) +% @as(c_ulong, @bitCast(@as(c_long, j.*.r.body_size)))))));
    _ = &n;
    if (!(n != null)) {
        warnx("%s:%d in %s: OOM%s", "src/submodules/beanstalkd/job.c", @as(c_int, 202), "job_copy", "");
        return @as([*c]Job, @ptrFromInt(@as(c_int, 0)));
    }
    _ = memcpy(@as(?*anyopaque, @ptrCast(n)), @as(?*const anyopaque, @ptrCast(j)), @sizeOf(Job) +% @as(c_ulong, @bitCast(@as(c_long, j.*.r.body_size))));
    job_list_reset(n);
    n.*.file = null;
    n.*.tube = null;
    _ = blk: {
        ...
        break :blk tube_iref(n.*.tube);
    };
    n.*.r.state = @as(byte, @bitCast(@as(i8, @truncate(Copy))));
    return n;
}
pub export fn job_state(arg_j: [*c]Job) [*c]const u8 {
    var j = arg_j;
    _ = &j;
    if (@as(c_int, @bitCast(@as(c_uint, j.*.r.state))) == Ready) return "ready";
    if (@as(c_int, @bitCast(@as(c_uint, j.*.r.state))) == Reserved) return "reserved";
    if (@as(c_int, @bitCast(@as(c_uint, j.*.r.state))) == Buried) return "buried";
    if (@as(c_int, @bitCast(@as(c_uint, j.*.r.state))) == Delayed) return "delayed";
    return "invalid";
}
pub export fn job_list_reset(arg_head: [*c]Job) void {
    var head = arg_head;
    _ = &head;
    head.*.prev = head;
    head.*.next = head;
}
pub export fn job_list_is_empty(arg_head: [*c]Job) c_int {
    var head = arg_head;
    _ = &head;
    return @intFromBool((head.*.next == head) and (head.*.prev == head));
}
pub export fn job_list_remove(arg_j: [*c]Job) [*c]Job {
    var j = arg_j;
    _ = &j;
    if (!(j != null)) return null;
    if (job_list_is_empty(j) != 0) return null;
    j.*.next.*.prev = j.*.prev;
    j.*.prev.*.next = j.*.next;
    job_list_reset(j);
    return j;
}
pub export fn job_list_insert(arg_head: [*c]Job, arg_j: [*c]Job) void {
    var head = arg_head;
    _ = &head;
    var j = arg_j;
    _ = &j;
    if (!(job_list_is_empty(j) != 0)) return;
    j.*.prev = head.*.prev;
    j.*.next = head;
    head.*.prev.*.next = j;
    head.*.prev = j;
}
pub export fn get_all_jobs_used() usize {
...
pub var next_id: uint64 = 1;
pub var cur_prime: c_int = 0;
pub var all_jobs_init: [12289][*c]Job = [1][*c]Job{
    null,
} ++ [1][*c]Job{null} ** 12288;
pub var all_jobs: [*c][*c]Job = @as([*c][*c]Job, @ptrCast(@alignCast(&all_jobs_init)));
pub var all_jobs_cap: usize = @as(usize, @bitCast(@as(c_long, @as(c_int, 12289))));
pub var all_jobs_used: usize = 0;
pub var hash_table_was_oom: c_int = 0;
pub fn rehash(arg_is_upscaling: c_int) callconv(.C) void {
    var is_upscaling = arg_is_upscaling;
    _ = &is_upscaling;
    var old: [*c][*c]Job = all_jobs;
    _ = &old;
    var old_cap: usize = all_jobs_cap;
    _ = &old_cap;
    var old_used: usize = all_jobs_used;
    _ = &old_used;
    var i: usize = undefined;
    _ = &i;
    var old_prime: c_int = cur_prime;
    _ = &old_prime;
    var d: c_int = if (is_upscaling != 0) @as(c_int, 1) else -@as(c_int, 1);
    _ = &d;
    if ((cur_prime + d) >= @as(c_int, 48)) return;
    if ((cur_prime + d) < @as(c_int, 0)) return;
    if ((is_upscaling != 0) and (hash_table_was_oom != 0)) return;
    cur_prime += d;
    all_jobs_cap = primes[@as(c_uint, @intCast(cur_prime))];
    all_jobs = @as([*c][*c]Job, @ptrCast(@alignCast(calloc(all_jobs_cap, @sizeOf([*c]Job)))));
    if (!(all_jobs != null)) {
        warnx("%s:%d in %s: Failed to allocate %zu new hash buckets%s", "src/submodules/beanstalkd/job.c", @as(c_int, 57), "rehash", all_jobs_cap, "");
    ...
    if (old != @as([*c][*c]Job, @ptrCast(@alignCast(&all_jobs_init)))) {
        free(@as(?*anyopaque, @ptrCast(old)));
    }
}
pub fn _get_job_hash_index(arg_job_id: uint64) callconv(.C) c_int {
    var job_id = arg_job_id;
    _ = &job_id;
    return @as(c_int, @bitCast(@as(c_uint, @truncate(job_id % all_jobs_cap))));
}
pub fn store_job(arg_j: [*c]Job) callconv(.C) void {
    var j = arg_j;
    _ = &j;
    var index_1: c_int = 0;
    _ = &index_1;
    index_1 = _get_job_hash_index(j.*.r.id);
    j.*.ht_next = (blk: {
        const tmp = index_1;
        if (tmp >= 0) break :blk all_jobs + @as(usize, @intCast(tmp)) else break :blk all_jobs - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    }).*;
    ...
    all_jobs_used +%= 1;
    if (all_jobs_used > (all_jobs_cap << @intCast(2))) {
        rehash(@as(c_int, 1));
    }
}
pub fn job_hash_free(arg_j: [*c]Job) callconv(.C) void {
    var j = arg_j;
    _ = &j;
    var slot: [*c][*c]Job = undefined;
    _ = &slot;
    slot = &(blk: {
        const tmp = _get_job_hash_index(j.*.r.id);
        if (tmp >= 0) break :blk all_jobs + @as(usize, @intCast(tmp)) else break :blk all_jobs - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    }).*;
    while ((slot.* != null) and (slot.* != j)) {
        slot = &slot.*.*.ht_next;
    }
    if (slot.* != null) {
        slot.* = slot.*.*.ht_next;
    ...
    if (all_jobs_used < (all_jobs_cap >> @intCast(4))) {
        rehash(@as(c_int, 0));
    }
}
