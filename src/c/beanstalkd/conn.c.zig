...
pub export fn make_conn(arg_fd: c_int, arg_start_state: u8, arg_use: [*c]Tube, arg_watch: [*c]Tube) [*c]Conn {
    var fd = arg_fd;
    _ = &fd;
    var start_state = arg_start_state;
    _ = &start_state;
    var use = arg_use;
    _ = &use;
    var watch = arg_watch;
    _ = &watch;
    var c: [*c]Conn = @as([*c]Conn, @ptrCast(@alignCast(zalloc(@as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(Conn)))))))));
    _ = &c;
    if (!(c != null)) {
        warn("%s:%d in %s: OOM%s", "src/submodules/beanstalkd/conn.c", @as(c_int, 39), "make_conn", "");
        return null;
    }
    ms_init(&c.*.watch, @as(ms_event_fn, @ptrCast(@alignCast(&on_watch))), @as(ms_event_fn, @ptrCast(@alignCast(&on_ignore))));
    if (!(ms_append(&c.*.watch, @as(?*anyopaque, @ptrCast(watch))) != 0)) {
        free(@as(?*anyopaque, @ptrCast(c)));
        warn("%s:%d in %s: OOM%s", "src/submodules/beanstalkd/conn.c", @as(c_int, 46), "make_conn", "");
        return null;
    }
    _ = blk: {
        _ = blk_1: {
...
pub export fn count_cur_conns() c_int {
    return cur_conn_ct;
}
pub export fn count_tot_conns() uint {
    return tot_conn_ct;
}
pub export fn count_cur_producers() c_int {
    return cur_producer_ct;
}
pub export fn count_cur_workers() c_int {
...
pub export fn conn_less(arg_ca: ?*anyopaque, arg_cb: ?*anyopaque) c_int {
    var ca = arg_ca;
    _ = &ca;
    var cb = arg_cb;
    _ = &cb;
    var a: [*c]Conn = @as([*c]Conn, @ptrCast(@alignCast(ca)));
    _ = &a;
    var b: [*c]Conn = @as([*c]Conn, @ptrCast(@alignCast(cb)));
    _ = &b;
    return @intFromBool(a.*.tickat < b.*.tickat);
}
pub export fn conn_setpos(arg_c: ?*anyopaque, arg_i: usize) void {
    var c = arg_c;
    _ = &c;
    var i = arg_i;
    _ = &i;
    @as([*c]Conn, @ptrCast(@alignCast(c))).*.tickpos = i;
}
pub export fn connsched(arg_c: [*c]Conn) void {
    var c = arg_c;
    _ = &c;
    if (c.*.in_conns != 0) {
        _ = heapremove(&c.*.srv.*.conns, c.*.tickpos);
        c.*.in_conns = 0;
    }
    c.*.tickat = conntickat(c);
    if (c.*.tickat != 0) {
        _ = heapinsert(&c.*.srv.*.conns, @as(?*anyopaque, @ptrCast(c)));
        c.*.in_conns = 1;
    }
}
pub export fn connclose(arg_c: [*c]Conn) void {
    var c = arg_c;
    _ = &c;
    _ = sockwant(&c.*.sock, @as(c_int, 0));
    _ = close(c.*.sock.fd);
    if (verbose != 0) {
        _ = printf("close %d\n", c.*.sock.fd);
    }
    ...
    c.*.in_job_read = 0;
    if ((@as(c_int, @bitCast(@as(c_uint, c.*.type))) & @as(c_int, 1)) != 0) {
        cur_producer_ct -= 1;
    }
    if ((@as(c_int, @bitCast(@as(c_uint, c.*.type))) & @as(c_int, 2)) != 0) {
        cur_worker_ct -= 1;
    }
    cur_conn_ct -= 1;
    remove_waiting_conn(c);
    if (has_reserved_job(c) != 0) {
...
pub export fn connsetproducer(arg_c: [*c]Conn) void {
    var c = arg_c;
    _ = &c;
    if ((@as(c_int, @bitCast(@as(c_uint, c.*.type))) & @as(c_int, 1)) != 0) return;
    c.*.type |= @as(u8, @bitCast(@as(i8, @truncate(@as(c_int, 1)))));
    cur_producer_ct += 1;
}
pub export fn connsetworker(arg_c: [*c]Conn) void {
    var c = arg_c;
    _ = &c;
    if ((@as(c_int, @bitCast(@as(c_uint, c.*.type))) & @as(c_int, 2)) != 0) return;
    c.*.type |= @as(u8, @bitCast(@as(i8, @truncate(@as(c_int, 2)))));
    cur_worker_ct += 1;
}
pub export fn connsoonestjob(arg_c: [*c]Conn) [*c]Job {
    var c = arg_c;
    _ = &c;
    if (c.*.soonest_job != @as([*c]Job, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return c.*.soonest_job;
    var j: [*c]Job = null;
    _ = &j;
    {
        j = c.*.reserved_jobs.next;
        while (j != (&c.*.reserved_jobs)) : (j = j.*.next) {
            conn_set_soonestjob(c, j);
        }
    }
    return c.*.soonest_job;
}
pub export fn conndeadlinesoon(arg_c: [*c]Conn) c_int {
    var c = arg_c;
    _ = &c;
    var t: int64 = nanoseconds();
    _ = &t;
    var j: [*c]Job = connsoonestjob(c);
    _ = &j;
    return @intFromBool((j != null) and (t >= (j.*.r.deadline_at - @as(int64, @bitCast(@as(c_long, @as(c_int, 1000000000)))))));
}
pub export fn conn_ready(arg_c: [*c]Conn) c_int {
    var c = arg_c;
    _ = &c;
    ...
    _ = &i;
    {
        ...
        while (i < c.*.watch.len) : (i +%= 1) {
            if (@as([*c]Tube, @ptrCast(@alignCast(c.*.watch.items[i]))).*.ready.len != 0) return 1;
        }
    }
    return 0;
}
pub export fn conn_reserve_job(arg_c: [*c]Conn, arg_j: [*c]Job) void {
    var c = arg_c;
    _ = &c;
    var j = arg_j;
    _ = &j;
    ...
    j.*.r.state = @as(byte, @bitCast(@as(i8, @truncate(Reserved))));
    job_list_insert(&c.*.reserved_jobs, j);
    j.*.reserver = @as(?*anyopaque, @ptrCast(c));
    c.*.pending_timeout = -@as(c_int, 1);
    conn_set_soonestjob(c, j);
}
...
pub var cur_conn_ct: c_int = 0;
pub var cur_worker_ct: c_int = 0;
pub var cur_producer_ct: c_int = 0;
pub var tot_conn_ct: uint = 0;
pub fn on_watch(arg_a: [*c]Ms, arg_t: [*c]Tube, arg_i: usize) callconv(.C) void {
    var a = arg_a;
    _ = &a;
    var t = arg_t;
    _ = &t;
...
pub fn on_ignore(arg_a: [*c]Ms, arg_t: [*c]Tube, arg_i: usize) callconv(.C) void {
    var a = arg_a;
    _ = &a;
    var t = arg_t;
    _ = &t;
...
pub fn has_reserved_job(arg_c: [*c]Conn) callconv(.C) c_int {
    var c = arg_c;
    _ = &c;
    return @intFromBool(!(job_list_is_empty(&c.*.reserved_jobs) != 0));
}
pub fn conntickat(arg_c: [*c]Conn) callconv(.C) int64 {
    var c = arg_c;
    _ = &c;
    var margin: c_int = 0;
    _ = &margin;
    ...
    _ = &should_timeout;
    var t: int64 = @as(c_long, 9223372036854775807);
    _ = &t;
    if ((@as(c_int, @bitCast(@as(c_uint, c.*.type))) & @as(c_int, 4)) != 0) {
        margin = @as(c_int, 1000000000);
    }
    if (has_reserved_job(c) != 0) {
        t = (connsoonestjob(c).*.r.deadline_at - nanoseconds()) - @as(int64, @bitCast(@as(c_long, margin)));
        should_timeout = 1;
    }
    if (c.*.pending_timeout >= @as(c_int, 0)) {
        t = if (t < (@as(int64, @bitCast(@as(c_long, c.*.pending_timeout))) * @as(int64, @bitCast(@as(c_long, @as(c_int, 1000000000)))))) t else @as(int64, @bitCast(@as(c_long, c.*.pending_timeout))) * @as(int64, @bitCast(@as(c_long, @as(c_int, 1000000000))));
        should_timeout = 1;
    }
    if (should_timeout != 0) {
...
pub fn conn_set_soonestjob(arg_c: [*c]Conn, arg_j: [*c]Job) callconv(.C) void {
    var c = arg_c;
    _ = &c;
    var j = arg_j;
    _ = &j;
    if (!(c.*.soonest_job != null) or (j.*.r.deadline_at < c.*.soonest_job.*.r.deadline_at)) {
        c.*.soonest_job = j;
    }
}
...
pub const SAFETY_MARGIN = @import("std").zig.c_translation.promoteIntLiteral(c_int, 1000000000, .decimal);
