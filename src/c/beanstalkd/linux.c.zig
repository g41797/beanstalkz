...
pub export fn sockinit() c_int {
    epfd = epoll_create(@as(c_int, 1));
    if (epfd == -@as(c_int, 1)) {
        warn("%s:%d in %s: epoll_create%s", "src/submodules/beanstalkd/linux.c", @as(c_int, 24), "sockinit", "");
        return -@as(c_int, 1);
    }
    return 0;
}
...
pub export fn sockwant(arg_s: [*c]Socket, arg_rw: c_int) c_int {
    var s = arg_s;
    _ = &s;
    var rw = arg_rw;
    _ = &rw;
    ...
    _ = &op;
    if (!(s.*.added != 0) and !(rw != 0)) {
        return 0;
    } else if (!(s.*.added != 0) and (rw != 0)) {
        s.*.added = 1;
        op = 1;
    } else if (!(rw != 0)) {
        op = 2;
    ...
    var ev: struct_epoll_event = struct_epoll_event{
        .events = @as(u32, @bitCast(@as(c_int, 0))),
        .data = @import("std").mem.zeroes(epoll_data_t),
    };
    _ = &ev;
    while (true) {
        switch (rw) {
            @as(c_int, 114) => {
                ev.events = @as(u32, @bitCast(EPOLLIN));
                break;
            },
            @as(c_int, 119) => {
                ev.events = @as(u32, @bitCast(EPOLLOUT));
                break;
            },
            else => {},
        }
    ...
    ev.events |= @as(u32, @bitCast(EPOLLRDHUP | EPOLLPRI));
    ev.data.ptr = @as(?*anyopaque, @ptrCast(s));
    return epoll_ctl(epfd, op, s.*.fd, &ev);
}
pub export fn socknext(arg_s: [*c][*c]Socket, arg_timeout: int64) c_int {
    var s = arg_s;
    _ = &s;
    ...
    _ = &ev;
    r = epoll_wait(epfd, &ev, @as(c_int, 1), @as(c_int, @bitCast(@as(c_int, @truncate(@divTrunc(timeout, @as(int64, @bitCast(@as(c_long, @as(c_int, 1000000))))))))));
    if ((r == -@as(c_int, 1)) and (__errno_location().* != @as(c_int, 4))) {
        warn("%s:%d in %s: epoll_wait%s", "src/submodules/beanstalkd/linux.c", @as(c_int, 71), "socknext", "");
        exit(@as(c_int, 1));
    }
    if (r > @as(c_int, 0)) {
        s.* = @as([*c]Socket, @ptrCast(@alignCast(ev.data.ptr)));
        if ((ev.events & @as(u32, @bitCast(EPOLLHUP | EPOLLRDHUP))) != 0) {
            return 'h';
        } else if ((ev.events & @as(u32, @bitCast(EPOLLIN))) != 0) {
            return 'r';
        } else if ((ev.events & @as(u32, @bitCast(EPOLLOUT))) != 0) {
            return 'w';
        }
    }
    return 0;
...
pub const struct_stat = extern struct {
    ...
    st_atime: __time_t = @import("std").mem.zeroes(__time_t),
    st_atimensec: __syscall_ulong_t = @import("std").mem.zeroes(__syscall_ulong_t),
    st_mtime: __time_t = @import("std").mem.zeroes(__time_t),
    st_mtimensec: __syscall_ulong_t = @import("std").mem.zeroes(__syscall_ulong_t),
    st_ctime: __time_t = @import("std").mem.zeroes(__time_t),
    st_ctimensec: __syscall_ulong_t = @import("std").mem.zeroes(__syscall_ulong_t),
...
pub var epfd: c_int = @import("std").mem.zeroes(c_int);
...
pub const _POSIX_C_SOURCE = @as(c_long, 200112);
...
pub const __POSIX2_THIS_VERSION = @as(c_long, 200112);
