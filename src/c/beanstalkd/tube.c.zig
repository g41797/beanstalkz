...
pub export fn make_tube(arg_name: [*c]const u8) [*c]Tube {
    var name = arg_name;
    _ = &name;
    var t: [*c]Tube = @as([*c]Tube, @ptrCast(@alignCast(zalloc(@as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(Tube)))))))));
    _ = &t;
    if (!(t != null)) return null;
    _ = strncpy(@as([*c]u8, @ptrCast(@alignCast(&t.*.name))), name, @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 201)))));
    if (@as(c_int, @bitCast(@as(c_uint, t.*.name[@as(c_uint, @intCast(@as(c_int, 201) - @as(c_int, 1)))]))) != @as(c_int, '\x00')) {
        t.*.name[@as(c_uint, @intCast(@as(c_int, 201) - @as(c_int, 1)))] = '\x00';
        warnx("%s:%d in %s: truncating tube name%s", "src/submodules/beanstalkd/tube.c", @as(c_int, 18), "make_tube", "");
    }
    ...
    var j: Job = Job{
        .r = @import("std").mem.zeroes(Jobrec),
        .pad = @import("std").mem.zeroes([6]u8),
        .tube = null,
        .prev = null,
        .next = null,
        .ht_next = null,
        .heap_index = @import("std").mem.zeroes(usize),
        .file = null,
        .fnext = null,
        .fprev = null,
        .reserver = null,
...
pub export fn tube_dref(arg_t: [*c]Tube) void {
    var t = arg_t;
    _ = &t;
    if (!(t != null)) return;
    if (t.*.refs < @as(uint, @bitCast(@as(c_int, 1)))) {
        warnx("%s:%d in %s: refs is zero for tube: %s%s", "src/submodules/beanstalkd/tube.c", @as(c_int, 49), "tube_dref", @as([*c]u8, @ptrCast(@alignCast(&t.*.name))), "");
        return;
    }
    t.*.refs -%= 1;
    if (t.*.refs < @as(uint, @bitCast(@as(c_int, 1)))) {
...
pub export fn tube_iref(arg_t: [*c]Tube) void {
    var t = arg_t;
    _ = &t;
    if (!(t != null)) return;
    t.*.refs +%= 1;
}
pub export fn tube_find(arg_tubeset: [*c]Ms, arg_name: [*c]const u8) [*c]Tube {
    var tubeset = arg_tubeset;
    _ = &tubeset;
    var name = arg_name;
    _ = &name;
    ...
    {
        i = 0;
        while (i < tubeset.*.len) : (i +%= 1) {
            var t: [*c]Tube = @as([*c]Tube, @ptrCast(@alignCast(tubeset.*.items[i])));
            _ = &t;
            if (strncmp(@as([*c]u8, @ptrCast(@alignCast(&t.*.name))), name, @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 201))))) == @as(c_int, 0)) return t;
        }
    }
    return null;
}
pub export fn tube_find_or_make(arg_name: [*c]const u8) [*c]Tube {
    var name = arg_name;
    _ = &name;
    var t: [*c]Tube = tube_find(&tubes, name);
    _ = &t;
    if (t != null) return t;
...
pub fn tube_free(arg_t: [*c]Tube) callconv(.C) void {
    var t = arg_t;
    _ = &t;
    _ = ms_remove(&tubes, @as(?*anyopaque, @ptrCast(t)));
    free(@as(?*anyopaque, @ptrCast(t.*.ready.data)));
    free(@as(?*anyopaque, @ptrCast(t.*.delay.data)));
    ms_clear(&t.*.waiting_conns);
    free(@as(?*anyopaque, @ptrCast(t)));
}
pub fn make_and_insert_tube(arg_name: [*c]const u8) callconv(.C) [*c]Tube {
    var name = arg_name;
    _ = &name;
    ...
    t = make_tube(name);
    if (!(t != null)) return null;
    r = ms_append(&tubes, @as(?*anyopaque, @ptrCast(t)));
    if (!(r != 0)) return blk: {
        tube_dref(t);
        break :blk @as([*c]Tube, @ptrFromInt(@as(c_int, 0)));
    };
    return t;
}
