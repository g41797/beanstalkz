...
pub export fn waldirlock(arg_w: [*c]Wal) c_int {
    var w = arg_w;
    _ = &w;
    ...
    _ = &fd;
    var lk: struct_flock = undefined;
    _ = &lk;
    var path: [*c]u8 = undefined;
    _ = &path;
    var path_length: usize = undefined;
    _ = &path_length;
    path_length = (strlen(w.*.dir) +% strlen("/lock")) +% @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1))));
    if ((blk: {
        const tmp = @as([*c]u8, @ptrCast(@alignCast(malloc(path_length))));
        path = tmp;
        break :blk tmp;
    }) == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        warn("%s:%d in %s: malloc%s", "src/submodules/beanstalkd/walg.c", @as(c_int, 408), "waldirlock", "");
        return 0;
    }
    _ = snprintf(path, path_length, "%s/lock", w.*.dir);
    fd = open(path, @as(c_int, 1) | @as(c_int, 64), @as(c_int, 384));
    free(@as(?*anyopaque, @ptrCast(path)));
    if (fd == -@as(c_int, 1)) {
        warn("%s:%d in %s: open%s", "src/submodules/beanstalkd/walg.c", @as(c_int, 416), "waldirlock", "");
        return 0;
    }
    lk.l_type = 1;
    lk.l_whence = 0;
    lk.l_start = 0;
    lk.l_len = 0;
    r = fcntl(fd, @as(c_int, 6), &lk);
    if (r != 0) {
        warn("%s:%d in %s: fcntl%s", "src/submodules/beanstalkd/walg.c", @as(c_int, 426), "waldirlock", "");
        return 0;
    }
    return 1;
}
pub export fn walinit(arg_w: [*c]Wal, arg_list: [*c]Job) void {
    var w = arg_w;
    _ = &w;
    var list = arg_list;
    _ = &list;
    var min_1: c_int = undefined;
    _ = &min_1;
    min_1 = walscandir(w);
    walread(w, list, min_1);
    if (!(makenextfile(w) != 0)) {
        warnx("%s:%d in %s: makenextfile%s", "src/submodules/beanstalkd/walg.c", @as(c_int, 487), "walinit", "");
        exit(@as(c_int, 1));
    }
    w.*.cur = w.*.tail;
}
pub export fn walwrite(arg_w: [*c]Wal, arg_j: [*c]Job) c_int {
    var w = arg_w;
    _ = &w;
    var j = arg_j;
    _ = &j;
    ...
    _ = &r;
    if (!(w.*.use != 0)) return 1;
    if ((w.*.cur.*.resv > @as(c_int, 0)) or (usenext(w) != 0)) {
        if (j.*.file != null) {
...
pub export fn walmaint(arg_w: [*c]Wal) void {
    var w = arg_w;
    _ = &w;
    if (w.*.use != 0) {
        walcompact(w);
...
pub export fn walresvput(arg_w: [*c]Wal, arg_j: [*c]Job) c_int {
    var w = arg_w;
    _ = &w;
    var j = arg_j;
    _ = &j;
    var z: c_int = 0;
    _ = &z;
    z += @as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(c_int)))));
    z += @as(c_int, @bitCast(@as(c_uint, @truncate(strlen(@as([*c]u8, @ptrCast(@alignCast(&j.*.tube.*.name))))))));
    z += @as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(Jobrec)))));
    z += @as(c_int, @bitCast(j.*.r.body_size));
    z += @as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(c_int)))));
    z += @as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(Jobrec)))));
    return reserve(w, z);
}
pub export fn walresvupdate(arg_w: [*c]Wal) c_int {
    var w = arg_w;
    _ = &w;
    var z: c_int = 0;
    _ = &z;
...
pub export fn walgc(arg_w: [*c]Wal) void {
    var w = arg_w;
    _ = &w;
    var f: [*c]File = undefined;
    _ = &f;
    while ((w.*.head != null) and !(w.*.head.*.refs != 0)) {
        f = w.*.head;
        w.*.head = f.*.next;
        if (w.*.tail == f) {
            w.*.tail = f.*.next;
        ...
        w.*.nfile -= 1;
        _ = unlink(f.*.path);
        free(@as(?*anyopaque, @ptrCast(f.*.path)));
...
pub fn reserve(arg_w: [*c]Wal, arg_n: c_int) callconv(.C) c_int {
    var w = arg_w;
    _ = &w;
    var n = arg_n;
    _ = &n;
    ...
    if (w.*.cur.*.free >= n) {
        w.*.cur.*.free -= n;
        w.*.cur.*.resv += n;
        w.*.resv += @as(int64, @bitCast(@as(c_long, n)));
        return n;
    }
    r = needfree(w, n);
    if (r != n) {
        warnx("%s:%d in %s: needfree%s", "src/submodules/beanstalkd/walg.c", @as(c_int, 346), "reserve", "");
        return 0;
    }
    w.*.tail.*.free -= n;
    w.*.tail.*.resv += n;
...
pub fn walscandir(arg_w: [*c]Wal) callconv(.C) c_int {
    var w = arg_w;
    _ = &w;
    const base = struct {
        var static: [7:0]u8 = "binlog.".*;
    };
    _ = &base;
    const len = struct {
        const static: c_int = @as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf([8]u8) -% @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1))))))));
    };
    _ = &len;
    var d: ?*DIR = undefined;
    _ = &d;
    var e: [*c]struct_dirent = undefined;
    _ = &e;
    var min_1: c_int = @as(c_int, 1) << @intCast(30);
    _ = &min_1;
    var max: c_int = 0;
    _ = &max;
    ...
    _ = &p;
    d = opendir(w.*.dir);
    if (!(d != null)) return min_1;
    while ((blk: {
        const tmp = readdir(d);
        e = tmp;
        break :blk tmp;
    }) != null) {
        if (strncmp(@as([*c]u8, @ptrCast(@alignCast(&e.*.d_name))), @as([*c]u8, @ptrCast(@alignCast(&base.static))), @as(c_ulong, @bitCast(@as(c_long, len.static)))) == @as(c_int, 0)) {
            n = @as(c_int, @bitCast(@as(c_int, @truncate(strtol(@as([*c]u8, @ptrCast(@alignCast(&e.*.d_name))) + @as(usize, @bitCast(@as(isize, @intCast(len.static)))), &p, @as(c_int, 10))))));
            if ((p != null) and (@as(c_int, @bitCast(@as(c_uint, p.*))) == @as(c_int, '\x00'))) {
                if (n > max) {
                    max = n;
                }
                if (n < min_1) {
    ...
    _ = closedir(d);
    w.*.next = max + @as(c_int, 1);
    return min_1;
}
pub fn usenext(arg_w: [*c]Wal) callconv(.C) c_int {
    var w = arg_w;
    _ = &w;
    var f: [*c]File = undefined;
    _ = &f;
    f = w.*.cur;
    if (!(f.*.next != null)) {
        warnx("%s:%d in %s: there is no next wal file%s", "src/submodules/beanstalkd/walg.c", @as(c_int, 81), "usenext", "");
        return 0;
    }
    w.*.cur = f.*.next;
    filewclose(f);
...
pub fn ratio(arg_w: [*c]Wal) callconv(.C) c_int {
    var w = arg_w;
    _ = &w;
    var n: int64 = undefined;
    _ = &n;
    var d: int64 = undefined;
    _ = &d;
    d = w.*.alive + w.*.resv;
    n = (@as(int64, @bitCast(@as(c_long, w.*.nfile))) * @as(int64, @bitCast(@as(c_long, w.*.filesize)))) - d;
    if (!(d != 0)) return 0;
    return @as(c_int, @bitCast(@as(c_int, @truncate(@divTrunc(n, d)))));
}
pub fn walresvmigrate(arg_w: [*c]Wal, arg_j: [*c]Job) callconv(.C) c_int {
    var w = arg_w;
    _ = &w;
    var j = arg_j;
    _ = &j;
...
pub fn moveone(arg_w: [*c]Wal) callconv(.C) void {
    var w = arg_w;
    _ = &w;
    var j: [*c]Job = undefined;
    _ = &j;
    if ((w.*.head == w.*.cur) or (w.*.head.*.next == w.*.cur)) {
        return;
    }
    j = w.*.head.*.jlist.fnext;
    if (!(j != null) or (j == (&w.*.head.*.jlist))) {
        warnx("%s:%d in %s: head holds no jlist%s", "src/submodules/beanstalkd/walg.c", @as(c_int, 133), "moveone", "");
        return;
    }
...
pub fn walcompact(arg_w: [*c]Wal) callconv(.C) void {
    var w = arg_w;
    _ = &w;
    var r: c_int = undefined;
    _ = &r;
    {
        r = ratio(w);
        while (r >= @as(c_int, 2)) : (r -= 1) {
            moveone(w);
        }
    }
}
pub fn walsync(arg_w: [*c]Wal) callconv(.C) void {
    var w = arg_w;
    _ = &w;
    var now: int64 = undefined;
    _ = &now;
    now = nanoseconds();
    if ((w.*.wantsync != 0) and (now >= (w.*.lastsync + w.*.syncrate))) {
        w.*.lastsync = now;
        if (fsync(w.*.cur.*.fd) == -@as(c_int, 1)) {
            warn("%s:%d in %s: fsync%s", "src/submodules/beanstalkd/walg.c", @as(c_int, 168), "walsync", "");
        }
    }
}
pub fn makenextfile(arg_w: [*c]Wal) callconv(.C) c_int {
    var w = arg_w;
    _ = &w;
    var f: [*c]File = undefined;
    _ = &f;
    f = @as([*c]File, @ptrCast(@alignCast(zalloc(@as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(File)))))))));
    if (!(f != null)) {
        warnx("%s:%d in %s: OOM%s", "src/submodules/beanstalkd/walg.c", @as(c_int, 217), "makenextfile", "");
        return 0;
    }
    if (!(fileinit(f, w, w.*.next) != 0)) {
        free(@as(?*anyopaque, @ptrCast(f)));
        warnx("%s:%d in %s: OOM%s", "src/submodules/beanstalkd/walg.c", @as(c_int, 223), "makenextfile", "");
        return 0;
    }
    filewopen(f);
    if (!(f.*.iswopen != 0)) {
...
pub fn moveresv(arg_to: [*c]File, arg_from: [*c]File, arg_n: c_int) callconv(.C) void {
    var to = arg_to;
    _ = &to;
    var from = arg_from;
    _ = &from;
...
pub fn needfree(arg_w: [*c]Wal, arg_n: c_int) callconv(.C) c_int {
    var w = arg_w;
    _ = &w;
    var n = arg_n;
    _ = &n;
    if (w.*.tail.*.free >= n) return n;
    if (makenextfile(w) != 0) return n;
    return 0;
}
pub fn balancerest(arg_w: [*c]Wal, arg_b: [*c]File, arg_n: c_int) callconv(.C) c_int {
    var w = arg_w;
    _ = &w;
    var b = arg_b;
    _ = &b;
    ...
    _ = &r;
    const z = struct {
        const static: c_int = @as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(c_int) +% @sizeOf(Jobrec)))));
    };
    _ = &z;
    if (!(b != null)) return 1;
    rest = b.*.resv - n;
    r = @import("std").zig.c_translation.signedRemainder(rest, z.static);
    if (r == @as(c_int, 0)) return balancerest(w, b.*.next, @as(c_int, 0));
    c = z.static - r;
    if ((w.*.tail.*.resv >= c) and (b.*.free >= c)) {
        moveresv(b, w.*.tail, c);
        return balancerest(w, b.*.next, @as(c_int, 0));
    }
    if (needfree(w, r) != r) {
        warnx("%s:%d in %s: needfree%s", "src/submodules/beanstalkd/walg.c", @as(c_int, 287), "balancerest", "");
        return 0;
    }
    moveresv(w.*.tail, b, r);
    return balancerest(w, b.*.next, @as(c_int, 0));
}
pub fn balance(arg_w: [*c]Wal, arg_n: c_int) callconv(.C) c_int {
    var w = arg_w;
    _ = &w;
    var n = arg_n;
    _ = &n;
    while (w.*.cur.*.resv < n) {
        var m: c_int = w.*.cur.*.resv;
        _ = &m;
        var r: c_int = needfree(w, m);
        _ = &r;
        if (r != m) {
            warnx("%s:%d in %s: needfree%s", "src/submodules/beanstalkd/walg.c", @as(c_int, 315), "balance", "");
            return 0;
        }
...
pub export fn walread(arg_w: [*c]Wal, arg_list: [*c]Job, arg_min_1: c_int) void {
    var w = arg_w;
    _ = &w;
    var list = arg_list;
    _ = &list;
    ...
    _ = &err;
    {
        i = min_1;
        while (i < w.*.next) : (i += 1) {
            var f: [*c]File = @as([*c]File, @ptrCast(@alignCast(zalloc(@as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(File)))))))));
            _ = &f;
            if (!(f != null)) {
                warnx("%s:%d in %s: OOM%s", "src/submodules/beanstalkd/walg.c", @as(c_int, 445), "walread", "");
                exit(@as(c_int, 1));
            }
            if (!(fileinit(f, w, i) != 0)) {
                free(@as(?*anyopaque, @ptrCast(f)));
                warnx("%s:%d in %s: OOM%s", "src/submodules/beanstalkd/walg.c", @as(c_int, 451), "walread", "");
                exit(@as(c_int, 1));
            }
            var fd: c_int = open(f.*.path, @as(c_int, 0));
            _ = &fd;
            if (fd < @as(c_int, 0)) {
                warn("%s:%d in %s: open %s%s", "src/submodules/beanstalkd/walg.c", @as(c_int, 457), "walread", f.*.path, "");
                free(@as(?*anyopaque, @ptrCast(f.*.path)));
                free(@as(?*anyopaque, @ptrCast(f)));
                continue;
            }
            ...
            if (close(fd) == -@as(c_int, 1)) {
                warn("%s:%d in %s: close%s", "src/submodules/beanstalkd/walg.c", @as(c_int, 467), "walread", "");
            }
        }
    }
    if (err != 0) {
