...
pub const DefaultPort = 11300;
/// Default tube name used by beanstalkd.
pub const DefaultTube = "default";
...
/// "Constructor" of beanstalkd client.
/// Arguments:
///     allocator:
///     io:
...
///     port: TCP/IP port,  for null - DefaultPort is used
///
/// Returns errors for:
///     - failed connection
///     - already existing connection
pub fn connect(cl: *Client, addr: ?[]const u8, port: ?u16) !void {
    _ = try cl.mutex.lock(cl.io);
    defer cl.mutex.unlock(cl.io);

    if (cl.connection != null) {
        return error.AlreadyConnected;
    }

    cl.connection = try Connection.connect(cl.*.allocator, cl.*.io, addr, port);

    cl.connection.?.prepareForSendRecv() catch unreachable;

    return;
}

pub fn disconnect(cl: *Client) void {
    cl.mutex.lock(cl.io) catch {};
    defer cl.mutex.unlock(cl.io);
...
fn print_line(cl: *Client, comptime fmt: []const u8, args: anytype) !void {
    ...
    try cl.connection.?.writer.?.interface.print(fmt, args);
    try cl.connection.?.writer.?.interface.print("\r\n", .{});
}
...
fn print_buffer(cl: *Client, buffer: []const u8) !void {
    ...
    try cl.connection.?.writer.?.interface.writeAll(buffer);
    try cl.connection.?.writer.?.interface.print("\r\n", .{});
}
...
fn flush(cl: *Client) !void {
    ...
    try cl.connection.?.writer.?.interface.flush();
}
...
// 0 - for \r\n only.
fn read_line(cl: *Client, buffer: []u8) !usize {// 2DO - move implementation to Connection
    if (cl.connection == null) {
        return ReturnedError.CommunicationFailure;
    }
    ...
    for (0..buffer.len) |indx| {
        ...
        const one: usize = cl.connection.?.reader.?.interface.readSliceShort(&input) catch 0;
...
fn read_buffer(cl: *Client, buffer: []u8, len: usize) !void {
    ...
    if (len > 0) {
        try cl.connection.?.reader.?.interface.readSliceAll(buffer[0..len]);
    }
