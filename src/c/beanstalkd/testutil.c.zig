...
pub export fn cttest_allocf() void {
    var got: [*c]u8 = undefined;
    _ = &got;
    got = fmtalloc(@as([*c]u8, @ptrCast(@volatileCast(@constCast("hello, %s %d")))), "world", @as(c_int, 5));
    while (true) {
        if (!(strcmp("hello, world 5", got) == @as(c_int, 0))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 16), "%s", "test: strcmp(\"hello, world 5\", got) == 0");
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 16), "got \"%s\"", got);
            ctfailnow();
        }
        if (!false) break;
    }
    free(@as(?*anyopaque, @ptrCast(got)));
}
pub export fn cttest_opt_none() void {
    var args: [1][*c]u8 = [1][*c]u8{
        null,
    };
    _ = &args;
    optparse(&srv, @as([*c][*c]u8, @ptrCast(@alignCast(&args))));
    while (true) {
        if (!(strcmp(srv.port, "11300") == @as(c_int, 0))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 28), "%s", "test: strcmp(srv.port, Portdef) == 0");
            ctfailnow();
        }
        if (!false) break;
    }
    while (true) {
        if (!(srv.addr == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 29), "%s", "test: srv.addr == NULL");
            ctfailnow();
        }
        if (!false) break;
    }
    while (true) {
        if (!(job_data_size_limit == @as(usize, @bitCast(@as(c_long, (@as(c_int, 1) << @intCast(16)) - @as(c_int, 1)))))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 30), "%s", "test: job_data_size_limit == JOB_DATA_SIZE_LIMIT_DEFAULT");
            ctfailnow();
        }
        if (!false) break;
    }
    while (true) {
        if (!(srv.wal.filesize == Filesizedef)) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 31), "%s", "test: srv.wal.filesize == Filesizedef");
            ctfailnow();
        }
        if (!false) break;
    }
    while (true) {
        if (!(srv.wal.wantsync == @as(c_int, 1))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 32), "%s", "test: srv.wal.wantsync == 1");
            ctfailnow();
        }
        if (!false) break;
    }
    while (true) {
        if (!(srv.wal.syncrate == @as(int64, @bitCast(@as(c_long, @as(c_int, 50) * @as(c_int, 1000000)))))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 33), "%s", "test: srv.wal.syncrate == DEFAULT_FSYNC_MS*1000000");
            ctfailnow();
        }
        if (!false) break;
    }
    while (true) {
        if (!(srv.user == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 34), "%s", "test: srv.user == NULL");
            ctfailnow();
        }
        if (!false) break;
    }
    while (true) {
        if (!(srv.wal.dir == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 35), "%s", "test: srv.wal.dir == NULL");
            ctfailnow();
        }
        if (!false) break;
    }
    while (true) {
        if (!(srv.wal.use == @as(c_int, 0))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 36), "%s", "test: srv.wal.use == 0");
            ctfailnow();
        }
        if (!false) break;
    }
    while (true) {
        if (!(verbose == @as(c_int, 0))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 37), "%s", "test: verbose == 0");
            ctfailnow();
        }
        if (!false) break;
    }
}
pub fn success() callconv(.C) void {
    _exit(@as(c_int, 0));
}
pub export fn cttest_optminus() void {
    ...
    while (true) {
        if (!(@as(c_int, 0) != 0)) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 56), "%s", "test: 0");
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 56), "optparse failed to call exit");
            ctfailnow();
        }
        if (!false) break;
    }
}
pub export fn cttest_optp() void {
    var args: [2][*c]u8 = [2][*c]u8{
        "-p1234",
        null,
    };
    ...
    optparse(&srv, @as([*c][*c]u8, @ptrCast(@alignCast(&args))));
    while (true) {
        if (!(strcmp(srv.port, "1234") == @as(c_int, 0))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 68), "%s", "test: strcmp(srv.port, \"1234\") == 0");
            ctfailnow();
        }
        if (!false) break;
    }
}
pub export fn cttest_optl() void {
    var args: [2][*c]u8 = [2][*c]u8{
        "-llocalhost",
        null,
    };
    ...
    optparse(&srv, @as([*c][*c]u8, @ptrCast(@alignCast(&args))));
    while (true) {
        if (!(strcmp(srv.addr, "localhost") == @as(c_int, 0))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 80), "%s", "test: strcmp(srv.addr, \"localhost\") == 0");
            ctfailnow();
        }
        if (!false) break;
    }
}
pub export fn cttest_optlseparate() void {
    ...
    while (true) {
        if (!(strcmp(srv.addr, "localhost") == @as(c_int, 0))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 93), "%s", "test: strcmp(srv.addr, \"localhost\") == 0");
            ctfailnow();
        }
        if (!false) break;
    }
}
pub export fn cttest_optz() void {
    var args: [2][*c]u8 = [2][*c]u8{
        "-z1234",
        null,
    };
    ...
    optparse(&srv, @as([*c][*c]u8, @ptrCast(@alignCast(&args))));
    while (true) {
        if (!(job_data_size_limit == @as(usize, @bitCast(@as(c_long, @as(c_int, 1234)))))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 105), "%s", "test: job_data_size_limit == 1234");
            ctfailnow();
        }
        if (!false) break;
    }
}
pub export fn cttest_optz_more_than_max() void {
    var args: [2][*c]u8 = [2][*c]u8{
        "-z1073741825",
        null,
    };
    ...
    optparse(&srv, @as([*c][*c]u8, @ptrCast(@alignCast(&args))));
    while (true) {
        if (!(job_data_size_limit == @as(usize, @bitCast(@as(c_long, @as(c_int, 1073741824)))))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 117), "%s", "test: job_data_size_limit == 1073741824");
            ctfailnow();
        }
        if (!false) break;
    }
}
pub export fn cttest_opts() void {
    var args: [2][*c]u8 = [2][*c]u8{
        "-s1234",
        null,
    };
    ...
    optparse(&srv, @as([*c][*c]u8, @ptrCast(@alignCast(&args))));
    while (true) {
        if (!(srv.wal.filesize == @as(c_int, 1234))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 129), "%s", "test: srv.wal.filesize == 1234");
            ctfailnow();
        }
        if (!false) break;
    }
}
pub export fn cttest_optf() void {
    var args: [2][*c]u8 = [2][*c]u8{
        "-f1234",
        null,
    };
    ...
    optparse(&srv, @as([*c][*c]u8, @ptrCast(@alignCast(&args))));
    while (true) {
        if (!(srv.wal.syncrate == @as(int64, @bitCast(@as(c_long, @as(c_int, 1234000000)))))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 141), "%s", "test: srv.wal.syncrate == 1234000000");
            ctfailnow();
        }
        if (!false) break;
    }
    while (true) {
        if (!(srv.wal.wantsync == @as(c_int, 1))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 142), "%s", "test: srv.wal.wantsync == 1");
            ctfailnow();
        }
        if (!false) break;
    }
...
pub export fn cttest_optF() void {
    ...
    optparse(&srv, @as([*c][*c]u8, @ptrCast(@alignCast(&args))));
    while (true) {
        if (!(srv.wal.wantsync == @as(c_int, 0))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 155), "%s", "test: srv.wal.wantsync == 0");
            ctfailnow();
        }
        if (!false) break;
    }
}
pub export fn cttest_optu() void {
    var args: [2][*c]u8 = [2][*c]u8{
        "-ukr",
        null,
    };
    ...
    optparse(&srv, @as([*c][*c]u8, @ptrCast(@alignCast(&args))));
    while (true) {
        if (!(strcmp(srv.user, "kr") == @as(c_int, 0))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 167), "%s", "test: strcmp(srv.user, \"kr\") == 0");
            ctfailnow();
        }
        if (!false) break;
    }
}
pub export fn cttest_optb() void {
    var args: [2][*c]u8 = [2][*c]u8{
        "-bfoo",
        null,
    };
    ...
    optparse(&srv, @as([*c][*c]u8, @ptrCast(@alignCast(&args))));
    while (true) {
        if (!(strcmp(srv.wal.dir, "foo") == @as(c_int, 0))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 179), "%s", "test: strcmp(srv.wal.dir, \"foo\") == 0");
            ctfailnow();
        }
        if (!false) break;
    }
    while (true) {
        if (!(srv.wal.use == @as(c_int, 1))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 180), "%s", "test: srv.wal.use == 1");
            ctfailnow();
        }
        if (!false) break;
    }
}
pub export fn cttest_optV() void {
    var args: [2][*c]u8 = [2][*c]u8{
        "-V",
        null,
    };
    ...
    while (true) {
        if (!(verbose == @as(c_int, 1))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 192), "%s", "test: verbose == 1");
            ctfailnow();
        }
        if (!false) break;
    }
}
pub export fn cttest_optV_V() void {
    var args: [3][*c]u8 = [3][*c]u8{
        "-V",
        "-V",
        null,
    ...
    while (true) {
        if (!(verbose == @as(c_int, 2))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 205), "%s", "test: verbose == 2");
            ctfailnow();
        }
        if (!false) break;
    }
}
pub export fn cttest_optVVV() void {
    var args: [2][*c]u8 = [2][*c]u8{
        "-VVV",
        null,
    };
    ...
    optparse(&srv, @as([*c][*c]u8, @ptrCast(@alignCast(&args))));
    while (true) {
        if (!(verbose == @as(c_int, 3))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 217), "%s", "test: verbose == 3");
            ctfailnow();
        }
        if (!false) break;
    }
}
pub export fn cttest_optVFVu() void {
    var args: [2][*c]u8 = [2][*c]u8{
        "-VFVukr",
        null,
    };
    ...
    while (true) {
        if (!(verbose == @as(c_int, 2))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 229), "%s", "test: verbose == 2");
            ctfailnow();
        }
        if (!false) break;
    }
    while (true) {
        if (!(srv.wal.wantsync == @as(c_int, 0))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 230), "%s", "test: srv.wal.wantsync == 0");
            ctfailnow();
        }
        if (!false) break;
    }
    while (true) {
        if (!(strcmp(srv.user, "kr") == @as(c_int, 0))) {
            ctlogpn("src/submodules/beanstalkd/testutil.c", @as(c_int, 231), "%s", "test: strcmp(srv.user, \"kr\") == 0");
            ctfailnow();
        }
        if (!false) break;
    }
