...
pub export fn srv_acquire_wal(arg_s: [*c]Server) void {
    var s = arg_s;
    _ = &s;
    if (s.*.wal.use != 0) {
        if (!(waldirlock(&s.*.wal) != 0)) {
            warnx("%s:%d in %s: failed to lock wal dir %s%s", "src/submodules/beanstalkd/serv.c", @as(c_int, 24), "srv_acquire_wal", s.*.wal.dir, "");
            exit(@as(c_int, 10));
        }
        var list: Job = Job{
        ...
        walinit(&s.*.wal, &list);
        var ok: c_int = prot_replay(s, &list);
        _ = &ok;
        if (!(ok != 0)) {
            warnx("%s:%d in %s: failed to replay log%s", "src/submodules/beanstalkd/serv.c", @as(c_int, 33), "srv_acquire_wal", "");
            exit(@as(c_int, 1));
        }
    }
}
pub export fn srvserve(arg_s: [*c]Server) void {
    var s = arg_s;
    _ = &s;
    var sock: [*c]Socket = undefined;
    _ = &sock;
    if (sockinit() == -@as(c_int, 1)) {
        warnx("%s:%d in %s: sockinit%s", "src/submodules/beanstalkd/serv.c", @as(c_int, 45), "srvserve", "");
        exit(@as(c_int, 1));
    }
    s.*.sock.x = @as(?*anyopaque, @ptrCast(s));
    s.*.sock.f = @as(Handle, @ptrCast(@alignCast(&srvaccept)));
    s.*.conns.less = &conn_less;
    s.*.conns.setpos = &conn_setpos;
    if (sockwant(&s.*.sock, @as(c_int, 'r')) == -@as(c_int, 1)) {
        warn("%s:%d in %s: sockwant%s", "src/submodules/beanstalkd/serv.c", @as(c_int, 55), "srvserve", "");
        exit(@as(c_int, 2));
    }
    while (true) {
        var period: int64 = prottick(s);
        _ = &period;
        var rw: c_int = socknext(&sock, period);
        _ = &rw;
        if (rw == -@as(c_int, 1)) {
            warnx("%s:%d in %s: socknext%s", "src/submodules/beanstalkd/serv.c", @as(c_int, 65), "srvserve", "");
            exit(@as(c_int, 1));
        }
        if (rw != 0) {
            sock.*.f.?(sock.*.x, rw);
...
pub export fn srvaccept(arg_s: [*c]Server, arg_ev: c_int) void {
    var s = arg_s;
    _ = &s;
    var ev = arg_ev;
    _ = &ev;
    h_accept(s.*.sock.fd, @as(c_short, @bitCast(@as(c_short, @truncate(ev)))), s);
}
