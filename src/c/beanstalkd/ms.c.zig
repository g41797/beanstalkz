...
pub export fn ms_init(arg_a: [*c]Ms, arg_oninsert: ms_event_fn, arg_onremove: ms_event_fn) void {
    var a = arg_a;
    _ = &a;
    var oninsert = arg_oninsert;
    _ = &oninsert;
    var onremove = arg_onremove;
    _ = &onremove;
    a.*.len = blk: {
        const tmp = blk_1: {
...
pub export fn ms_clear(arg_a: [*c]Ms) void {
    var a = arg_a;
    _ = &a;
    while (ms_delete(a, @as(usize, @bitCast(@as(c_long, @as(c_int, 0))))) != 0) {}
    free(@as(?*anyopaque, @ptrCast(a.*.items)));
    ms_init(a, a.*.oninsert, a.*.onremove);
}
pub export fn ms_append(arg_a: [*c]Ms, arg_item: ?*anyopaque) c_int {
    var a = arg_a;
    _ = &a;
    var item = arg_item;
    _ = &item;
    if ((a.*.len >= a.*.cap) and !(grow(a) != 0)) return 0;
    a.*.items[blk: {
            const ref = &a.*.len;
            const tmp = ref.*;
            ref.* +%= 1;
    ...
    if (a.*.oninsert != null) {
        a.*.oninsert.?(a, item, a.*.len -% @as(usize, @bitCast(@as(c_long, @as(c_int, 1)))));
    }
    return 1;
}
pub export fn ms_remove(arg_a: [*c]Ms, arg_item: ?*anyopaque) c_int {
    var a = arg_a;
    _ = &a;
    var item = arg_item;
    _ = &item;
...
pub export fn ms_contains(arg_a: [*c]Ms, arg_item: ?*anyopaque) c_int {
    var a = arg_a;
    _ = &a;
    var item = arg_item;
    _ = &item;
...
pub export fn ms_take(arg_a: [*c]Ms) ?*anyopaque {
    var a = arg_a;
    _ = &a;
    var item: ?*anyopaque = undefined;
    _ = &item;
    if (!(a.*.len != 0)) return @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)));
    a.*.last = a.*.last % a.*.len;
    item = a.*.items[a.*.last];
    _ = ms_delete(a, a.*.last);
    a.*.last +%= 1;
    return item;
}
...
pub fn grow(arg_a: [*c]Ms) callconv(.C) c_int {
    var a = arg_a;
    _ = &a;
    var nitems: [*c]?*anyopaque = undefined;
    _ = &nitems;
    var ncap: usize = a.*.cap << @intCast(1);
    _ = &ncap;
    if (!(ncap != 0)) {
        ncap = 1;
    }
    nitems = @as([*c]?*anyopaque, @ptrCast(@alignCast(malloc(ncap *% @sizeOf(?*anyopaque)))));
    if (!(nitems != null)) return 0;
    _ = memcpy(@as(?*anyopaque, @ptrCast(nitems)), @as(?*const anyopaque, @ptrCast(a.*.items)), a.*.len *% @sizeOf(?*anyopaque));
    free(@as(?*anyopaque, @ptrCast(a.*.items)));
    a.*.items = nitems;
    a.*.cap = ncap;
    return 1;
}
pub fn ms_delete(arg_a: [*c]Ms, arg_i: usize) callconv(.C) c_int {
    var a = arg_a;
    _ = &a;
    var i = arg_i;
    _ = &i;
