...
pub export fn rawfalloc(arg_fd: c_int, arg_len: c_int) c_int {
    var fd = arg_fd;
    _ = &fd;
    var len = arg_len;
    _ = &len;
    const buf = struct {
        var static: [4096]u8 = [1]u8{
            0,
        } ++ [1]u8{0} ** 4095;
    };
    _ = &buf;
    ...
    _ = &w;
    {
        i = 0;
        while (i < len) : (i += w) {
            w = @as(c_int, @bitCast(@as(c_int, @truncate(write(fd, @as(?*const anyopaque, @ptrCast(@as([*c]u8, @ptrCast(@alignCast(&buf.static))))), @sizeOf([4096]u8))))));
            if (w == -@as(c_int, 1)) return __errno_location().*;
        }
    }
    _ = lseek(fd, @as(__off_t, @bitCast(@as(c_long, @as(c_int, 0)))), @as(c_int, 0));
    return 0;
}
...
pub export fn fileinit(arg_f: [*c]File, arg_w: [*c]Wal, arg_n: c_int) c_int {
    var f = arg_f;
    _ = &f;
    var w = arg_w;
    _ = &w;
    ...
    f.*.w = w;
    f.*.seq = n;
    f.*.path = fmtalloc(@as([*c]u8, @ptrCast(@volatileCast(@constCast("%s/binlog.%d")))), w.*.dir, n);
    return @intFromBool(!!(f.*.path != null));
}
pub export fn fileadd(arg_f: [*c]File, arg_w: [*c]Wal) [*c]Wal {
    var f = arg_f;
    _ = &f;
    var w = arg_w;
    _ = &w;
...
pub export fn fileincref(arg_f: [*c]File) void {
    var f = arg_f;
    _ = &f;
    if (!(f != null)) return;
    f.*.refs +%= 1;
}
pub export fn filedecref(arg_f: [*c]File) void {
    var f = arg_f;
    _ = &f;
    if (!(f != null)) return;
    f.*.refs -%= 1;
    if (f.*.refs < @as(uint, @bitCast(@as(c_int, 1)))) {
        walgc(f.*.w);
    }
}
pub export fn fileaddjob(arg_f: [*c]File, arg_j: [*c]Job) void {
    var f = arg_f;
    _ = &f;
    var j = arg_j;
    _ = &j;
    var h: [*c]Job = undefined;
    _ = &h;
    h = &f.*.jlist;
    if (!(h.*.fprev != null)) {
        h.*.fprev = h;
...
pub export fn filermjob(arg_f: [*c]File, arg_j: [*c]Job) void {
    var f = arg_f;
    _ = &f;
    var j = arg_j;
    _ = &j;
    ...
    j.*.fprev = null;
    j.*.file = null;
    f.*.w.*.alive -= @as(int64, @bitCast(@as(c_long, j.*.walused)));
    j.*.walused = 0;
    filedecref(f);
}
pub export fn fileread(arg_f: [*c]File, arg_list: [*c]Job) c_int {
    var f = arg_f;
    _ = &f;
    var list = arg_list;
    _ = &list;
    ...
    _ = &v;
    if (!(readfull(f, @as(?*anyopaque, @ptrCast(&v)), @as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(c_int))))), &err, @as([*c]u8, @ptrCast(@volatileCast(@constCast("version"))))) != 0)) {
        return err;
    }
    while (true) {
        switch (v) {
            @as(c_int, 7) => {
                fileincref(f);
                while (readrec(f, list, &err) != 0) {}
                filedecref(f);
                return err;
            },
            @as(c_int, 5) => {
                fileincref(f);
                while (readrec5(f, list, &err) != 0) {}
                filedecref(f);
                return err;
            },
            else => {},
...
pub export fn filewopen(arg_f: [*c]File) void {
    var f = arg_f;
    _ = &f;
    ...
    _ = &n;
    var ver: c_int = Walver;
    _ = &ver;
    fd = open(f.*.path, @as(c_int, 1) | @as(c_int, 64), @as(c_int, 256));
    if (fd < @as(c_int, 0)) {
        warn("%s:%d in %s: open %s%s", "src/submodules/beanstalkd/file.c", @as(c_int, 468), "filewopen", f.*.path, "");
        return;
    }
    r = falloc.?(fd, f.*.w.*.filesize);
    if (r != 0) {
        if (close(fd) == -@as(c_int, 1)) {
            warn("%s:%d in %s: close%s", "src/submodules/beanstalkd/file.c", @as(c_int, 475), "filewopen", "");
        }
        __errno_location().* = r;
        warn("%s:%d in %s: falloc %s%s", "src/submodules/beanstalkd/file.c", @as(c_int, 477), "filewopen", f.*.path, "");
        r = unlink(f.*.path);
        if (r != 0) {
            warn("%s:%d in %s: unlink %s%s", "src/submodules/beanstalkd/file.c", @as(c_int, 480), "filewopen", f.*.path, "");
        }
        return;
    }
    n = @as(c_int, @bitCast(@as(c_int, @truncate(write(fd, @as(?*const anyopaque, @ptrCast(&ver)), @sizeOf(c_int))))));
    if ((n < @as(c_int, 0)) or (@as(usize, @bitCast(@as(c_long, n))) < @sizeOf(c_int))) {
        warn("%s:%d in %s: write %s%s", "src/submodules/beanstalkd/file.c", @as(c_int, 487), "filewopen", f.*.path, "");
        if (close(fd) == -@as(c_int, 1)) {
            warn("%s:%d in %s: close%s", "src/submodules/beanstalkd/file.c", @as(c_int, 489), "filewopen", "");
        }
        return;
    }
    f.*.fd = fd;
...
pub export fn filewclose(arg_f: [*c]File) void {
    var f = arg_f;
    _ = &f;
    if (!(f != null)) return;
    if (!(f.*.iswopen != 0)) return;
    if (f.*.free != 0) {
        __errno_location().* = 0;
        if (ftruncate(f.*.fd, @as(__off_t, @bitCast(@as(c_long, f.*.w.*.filesize - f.*.free)))) != @as(c_int, 0)) {
            warn("%s:%d in %s: ftruncate%s", "src/submodules/beanstalkd/file.c", @as(c_int, 562), "filewclose", "");
        }
    }
    if (close(f.*.fd) == -@as(c_int, 1)) {
        warn("%s:%d in %s: close%s", "src/submodules/beanstalkd/file.c", @as(c_int, 566), "filewclose", "");
    }
    f.*.iswopen = 0;
    filedecref(f);
}
pub export fn filewrjobshort(arg_f: [*c]File, arg_j: [*c]Job) c_int {
    var f = arg_f;
    _ = &f;
    var j = arg_j;
    _ = &j;
    ...
    _ = &nl;
    nl = 0;
    r = @intFromBool((filewrite(f, j, @as(?*anyopaque, @ptrCast(&nl)), @as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(c_int)))))) != 0) and (filewrite(f, j, @as(?*anyopaque, @ptrCast(&j.*.r)), @as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(Jobrec)))))) != 0));
    if (!(r != 0)) return 0;
    if (@as(c_int, @bitCast(@as(c_uint, j.*.r.state))) == Invalid) {
        filermjob(j.*.file, j);
    }
    return r;
}
pub export fn filewrjobfull(arg_f: [*c]File, arg_j: [*c]Job) c_int {
    var f = arg_f;
    _ = &f;
    var j = arg_j;
    _ = &j;
    ...
    _ = &nl;
    fileaddjob(f, j);
    nl = @as(c_int, @bitCast(@as(c_uint, @truncate(strlen(@as([*c]u8, @ptrCast(@alignCast(&j.*.tube.*.name))))))));
    return @intFromBool((((filewrite(f, j, @as(?*anyopaque, @ptrCast(&nl)), @as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(c_int)))))) != 0) and (filewrite(f, j, @as(?*anyopaque, @ptrCast(@as([*c]u8, @ptrCast(@alignCast(&j.*.tube.*.name))))), nl) != 0)) and (filewrite(f, j, @as(?*anyopaque, @ptrCast(&j.*.r)), @as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(Jobrec)))))) != 0)) and (filewrite(f, j, @as(?*anyopaque, @ptrCast(j.*.body)), j.*.r.body_size) != 0));
}
...
// src/submodules/beanstalkd/file.c:231:17: warning: TODO implement translation of stmt class GotoStmtClass

// src/submodules/beanstalkd/file.c:157:1: warning: unable to translate function, demoted to extern
pub extern fn readrec(arg_f: [*c]File, arg_l: [*c]Job, arg_err: [*c]c_int) callconv(.C) c_int;
pub const struct_Jobrec5 = extern struct {
    id: uint64 = @import("std").mem.zeroes(uint64),
    pri: uint32 = @import("std").mem.zeroes(uint32),
    delay: uint64 = @import("std").mem.zeroes(uint64),
    ttr: uint64 = @import("std").mem.zeroes(uint64),
    body_size: int32 = @import("std").mem.zeroes(int32),
    created_at: uint64 = @import("std").mem.zeroes(uint64),
    deadline_at: uint64 = @import("std").mem.zeroes(uint64),
    reserve_ct: uint32 = @import("std").mem.zeroes(uint32),
    timeout_ct: uint32 = @import("std").mem.zeroes(uint32),
    release_ct: uint32 = @import("std").mem.zeroes(uint32),
    bury_ct: uint32 = @import("std").mem.zeroes(uint32),
    kick_ct: uint32 = @import("std").mem.zeroes(uint32),
    state: byte = @import("std").mem.zeroes(byte),
    pad: [1]u8 = @import("std").mem.zeroes([1]u8),
};
pub const Jobrec5 = struct_Jobrec5;
// src/submodules/beanstalkd/file.c:356:17: warning: TODO implement translation of stmt class GotoStmtClass

// src/submodules/beanstalkd/file.c:288:1: warning: unable to translate function, demoted to extern
pub extern fn readrec5(arg_f: [*c]File, arg_l: [*c]Job, arg_err: [*c]c_int) callconv(.C) c_int;
pub fn readfull(arg_f: [*c]File, arg_c: ?*anyopaque, arg_n: c_int, arg_err: [*c]c_int, arg_desc: [*c]u8) callconv(.C) c_int {
    var f = arg_f;
    _ = &f;
    var c = arg_c;
    _ = &c;
    ...
    var r: c_int = undefined;
    _ = &r;
    r = @as(c_int, @bitCast(@as(c_int, @truncate(read(f.*.fd, c, @as(usize, @bitCast(@as(c_long, n))))))));
    if (r == -@as(c_int, 1)) {
        warn("%s:%d in %s: read%s", "src/submodules/beanstalkd/file.c", @as(c_int, 428), "readfull", "");
        warnpos(f, @as(c_int, 0), @as([*c]u8, @ptrCast(@volatileCast(@constCast("error reading %s")))), desc);
        err.* = 1;
        return 0;
    }
    if (r != n) {
        warnpos(f, -r, @as([*c]u8, @ptrCast(@volatileCast(@constCast("unexpected EOF reading %d bytes (got %d): %s")))), n, r, desc);
        err.* = 1;
        return 0;
    }
    return r;
}
// src/submodules/beanstalkd/file.c:442:1: warning: TODO unable to translate variadic function, demoted to extern
pub extern fn warnpos(f: [*c]File, adj: c_int, fmt: [*c]u8, ...) void;
pub const Walver5: c_int = 5;
const enum_unnamed_11 = c_uint;
pub const Jobrec5size: c_int = 77;
const enum_unnamed_12 = c_uint;
pub fn filewrite(arg_f: [*c]File, arg_j: [*c]Job, arg_buf: ?*anyopaque, arg_len: c_int) callconv(.C) c_int {
    var f = arg_f;
    _ = &f;
    var j = arg_j;
    _ = &j;
    ...
    var r: c_int = undefined;
    _ = &r;
    r = @as(c_int, @bitCast(@as(c_int, @truncate(write(f.*.fd, buf, @as(usize, @bitCast(@as(c_long, len))))))));
    if (r != len) {
        warn("%s:%d in %s: write%s", "src/submodules/beanstalkd/file.c", @as(c_int, 508), "filewrite", "");
        return 0;
    }
    f.*.w.*.resv -= @as(int64, @bitCast(@as(c_long, r)));
    f.*.resv -= r;
    j.*.walresv -= r;
    j.*.walused += r;
    f.*.w.*.alive += @as(int64, @bitCast(@as(c_long, r)));
    return 1;
}
