...
pub export fn cttest_ms_append() void {
    var a: [*c]Ms = @as([*c]Ms, @ptrCast(@alignCast(zalloc(@as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(struct_Ms)))))))));
    _ = &a;
    ms_init(a, null, null);
    var i: c_int = 10;
    _ = &i;
    var ok: c_int = ms_append(a, @as(?*anyopaque, @ptrCast(&i)));
    _ = &ok;
    while (true) {
        if (!(a.*.len == @as(usize, @bitCast(@as(c_long, @as(c_int, 1)))))) {
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 19), "%s", "test: a->len == 1");
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 19), "a should contain one item");
            ctfailnow();
        }
        if (!false) break;
    }
    while (true) {
        if (!(ok != 0)) {
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 20), "%s", "test: ok");
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 20), "should be added");
            ctfailnow();
        }
        if (!false) break;
    }
    ok = ms_append(a, @as(?*anyopaque, @ptrCast(&i)));
    while (true) {
        if (!(a.*.len == @as(usize, @bitCast(@as(c_long, @as(c_int, 2)))))) {
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 23), "%s", "test: a->len == 2");
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 23), "a should contain two items");
            ctfailnow();
        }
        if (!false) break;
    }
    while (true) {
        if (!(ok != 0)) {
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 24), "%s", "test: ok");
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 24), "should be added");
            ctfailnow();
        }
        if (!false) break;
    }
    ms_clear(a);
    while (true) {
        if (!(a.*.len == @as(usize, @bitCast(@as(c_long, @as(c_int, 0)))))) {
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 27), "%s", "test: a->len == 0");
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 27), "a should be empty");
            ctfailnow();
        }
        if (!false) break;
    }
    free(@as(?*anyopaque, @ptrCast(a.*.items)));
    free(@as(?*anyopaque, @ptrCast(a)));
}
pub export fn cttest_ms_remove() void {
    var a: [*c]Ms = @as([*c]Ms, @ptrCast(@alignCast(zalloc(@as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(struct_Ms)))))))));
    _ = &a;
    ms_init(a, null, null);
    var i: c_int = 1;
    _ = &i;
    _ = ms_append(a, @as(?*anyopaque, @ptrCast(&i)));
    var j: c_int = 2;
    _ = &j;
    var ok: c_int = ms_remove(a, @as(?*anyopaque, @ptrCast(&j)));
    _ = &ok;
    while (true) {
        if (!!(ok != 0)) {
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 44), "%s", "test: !ok");
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 44), "j should not be removed");
            ctfailnow();
        }
        if (!false) break;
    }
    ok = ms_remove(a, @as(?*anyopaque, @ptrCast(&i)));
    while (true) {
        if (!(ok != 0)) {
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 47), "%s", "test: ok");
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 47), "i should be removed");
            ctfailnow();
        }
        if (!false) break;
    }
    ...
    while (true) {
        if (!!(ok != 0)) {
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 50), "%s", "test: !ok");
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 50), "i was already removed");
            ctfailnow();
        }
        if (!false) break;
    }
    while (true) {
        if (!(a.*.len == @as(usize, @bitCast(@as(c_long, @as(c_int, 0)))))) {
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 52), "%s", "test: a->len == 0");
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 52), "a should be empty");
            ctfailnow();
        }
        if (!false) break;
    }
...
pub export fn cttest_ms_contains() void {
    var a: [*c]Ms = @as([*c]Ms, @ptrCast(@alignCast(zalloc(@as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(struct_Ms)))))))));
    _ = &a;
    ms_init(a, null, null);
    var i: c_int = 1;
    _ = &i;
    _ = ms_append(a, @as(?*anyopaque, @ptrCast(&i)));
    var ok: c_int = ms_contains(a, @as(?*anyopaque, @ptrCast(&i)));
    _ = &ok;
    while (true) {
        if (!(ok != 0)) {
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 67), "%s", "test: ok");
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 67), "i should be in a");
            ctfailnow();
        }
        if (!false) break;
    }
    var j: c_int = 2;
    _ = &j;
    ok = ms_contains(a, @as(?*anyopaque, @ptrCast(&j)));
    while (true) {
        if (!!(ok != 0)) {
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 71), "%s", "test: !ok");
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 71), "j should not be in a");
            ctfailnow();
        }
        if (!false) break;
    }
...
pub export fn cttest_ms_clear_empty() void {
    var a: [*c]Ms = @as([*c]Ms, @ptrCast(@alignCast(zalloc(@as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(struct_Ms)))))))));
    _ = &a;
    ms_init(a, null, null);
    ms_clear(a);
    while (true) {
        if (!(a.*.len == @as(usize, @bitCast(@as(c_long, @as(c_int, 0)))))) {
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 86), "%s", "test: a->len == 0");
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 86), "a should be empty");
            ctfailnow();
        }
        if (!false) break;
    }
...
pub export fn cttest_ms_take() void {
    var a: [*c]Ms = @as([*c]Ms, @ptrCast(@alignCast(zalloc(@as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(struct_Ms)))))))));
    _ = &a;
    ms_init(a, null, null);
    var i: c_int = 10;
    ...
    _ = &j;
    _ = ms_append(a, @as(?*anyopaque, @ptrCast(&i)));
    _ = ms_append(a, @as(?*anyopaque, @ptrCast(&j)));
    var n: [*c]c_int = undefined;
    _ = &n;
    n = @as([*c]c_int, @ptrCast(@alignCast(ms_take(a))));
    while (true) {
        if (!(n == (&i))) {
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 105), "%s", "test: n == &i");
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 105), "n should point to i");
            ctfailnow();
        }
        if (!false) break;
    }
    ...
    while (true) {
        if (!(n == (&j))) {
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 108), "%s", "test: n == &j");
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 108), "n should point to j");
            ctfailnow();
        }
        if (!false) break;
    }
    ...
    while (true) {
        if (!(n == @as([*c]c_int, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 111), "%s", "test: n == NULL");
            ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 111), "n should be NULL; ms is empty");
            ctfailnow();
        }
        if (!false) break;
    }
...
pub export fn cttest_ms_take_sequence() void {
    var i: usize = undefined;
    _ = &i;
    var s: [6]c_int = [6]c_int{
        1,
        2,
        3,
        4,
    ...
    _ = &a;
    ms_init(a, null, null);
    var n: usize = @sizeOf([6]c_int) / @sizeOf(c_int);
    _ = &n;
    {
        i = 0;
        while (i < n) : (i +%= 1) {
            _ = ms_append(a, @as(?*anyopaque, @ptrCast(&s[i])));
        }
    }
    {
        i = 0;
        while (i < n) : (i +%= 1) {
            var got: [*c]c_int = @as([*c]c_int, @ptrCast(@alignCast(ms_take(a))));
            _ = &got;
            while (true) {
                if (!(got.* == e[i])) {
                    ctlogpn("src/submodules/beanstalkd/testms.c", @as(c_int, 133), "%s", "test: *got == e[i]");
                    ctfailnow();
                }
                if (!false) break;
            }
